...
<?php
	...
	function insertEmployee($params) {
        ...
        $sql="INSERT INTO `jobs` (`ID`, `title`, `company_name`, `company_desc`, `category`, `salary`, `location`,`vacancies`,`qualification`,`experience`,`shift`,`description`,`date_time`,`type`,`skills`) VALUES (NULL, '$title', '$company_name', '$company_desc', '$category', '$salary', '$location','$vacancies','$qualification','$experience','$shift','$description','$date_time','$type','$skills');";
	...
	function getRecords($params) {
       $qtot = mysqli_query($this->conn,"SET NAMES utf8"); 
		...
		if( !empty($params['searchPhrase']) ) {   
			...
			$where .=" OR company_name LIKE '".$params['searchPhrase']."%'";   
			$where .=" OR category LIKE '".$params['searchPhrase']."%'";   
			$where .=" OR company_desc LIKE '".$params['searchPhrase']."%'";   
...
function updateEmployee($params) {
   ...
   $sql = "UPDATE `jobs` SET `title` = '".$params["title"]."',
 ...
 die('{"msg":"Updated successfully","type":"success","error":"1"}');
	...
	function deleteEmployee($params) {
		...
		$data = array();
		//print_R($_POST);die;
		$sql = "delete from `jobs` WHERE ID='".$params["id"]."'";
